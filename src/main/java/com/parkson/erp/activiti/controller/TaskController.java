...
import com.parkson.erp.activiti.model.MyTask;
import com.parkson.erp.activiti.model.PageInfo;
import com.parkson.erp.activiti.model.ActUser;
import com.parkson.erp.activiti.service.LeaveService;
import com.parkson.erp.activiti.util.DateJsonValueProcessor;
...
//				.singleResult(); 
	...
	public ModelAndView audit_bz(String taskId,Integer leaveDays,String comment,Integer state,HttpServletResponse response,HttpSession session)throws Exception{
		//首先根据ID查询任务
		Task task=taskService.createTaskQuery() // 创建任务查询
		        .taskId(taskId) // 根据任务id查询
		        .singleResult();
		...
		//取出用户，角色信息
		ActUser currentUser=currentMemberShip.getUser();
